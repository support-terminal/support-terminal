...
import io.github.support.terminal.application.domains.monitoring.MonitoringTaskRepositoryTestConfig
...
class NotificationApiRepositoryTest {
    ...
    fun `edit monitoring task details`() {
        repository.add(MonitoringTaskDataSet.MonitoringTasks.MONITORING_TASK_1.details())
        repository.findById(MonitoringTaskDataSet.MonitoringTasks.MONITORING_TASK_1.details().id)
                ?. let {
                    val updated = it.merge(MonitoringTaskDataSet.MonitoringTasks.MONITORING_TASK_2.details())
                    repository.update(updated)
                    Assertions.assertEquals(MonitoringTaskDataSet.MonitoringTasks.MONITORING_TASK_1_UPDATE.details(), updated)
                } ?: throw IllegalArgumentException("Not found monitoring details")
            }
